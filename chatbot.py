...
import os
import agent
from translator import translate_tw, translate_cn
import asyncio
import re
...
class Chatbot:
    def __init__(self):
        ...
        if "history" not in st.session_state and \
           "messages" not in st.session_state and \
           "messages_display" not in st.session_state and \
           "check_message_for_understanding" not in st.session_state:
            st.session_state["history"] = []      # initialize the conversation history
            st.session_state["messages"] = []  # initialize the messages in simplified Chinese
            st.session_state["messages_display"] = []  # initialize the messages in traditional Chinese
            st.session_state["check_messages_for_understanding"] = [] # initialize the messages for checking student understanding
    ...
    # Process the first default start prompt (i.e. show the first guidance step)
    def process_start_prompt(self):
        if  st.session_state["is_first_user_prompt"]:        # if this is the first message from the user
            self.choice_made = False   # initialize user choice to False
            if st.session_state["current_num_guidance"] == 3:   # if there are different ways to solve the problem
                if st.session_state["current_solution_stage"] == 0:
                    # Check whether the calculation is from left to right
                    st.markdown(
                        """
                        <style>
                        /* Reduce bottom margin of the header text */
                        .custom-header {
                            font-size: 16px;
                            margin-bottom: -50px;  /* Adjust this value as needed */
                        }
                        /* Reduce top margin of the radio group */
                        div[role="radiogroup"] {
                            margin-top: 0px !important;
                        }
                        /* Move sidebar submit button to adjust space */
                        section[data-testid="stSidebar"] .stButton {
                            margin-top: 15px !important;  /* Reduced positive margin to move button slightly higher */
                        }
                        </style>
                        """,
                        unsafe_allow_html=True
                    )
                    # Add a markdown header with a custom class
                    st.markdown('<div class="custom-header">你選擇哪一個計算策略：</div>',
                                unsafe_allow_html=True)
                    option = st.radio(
                        ' ',
                        ('', 'A. 由左至右順序計算', 'B. 調換運算次序'),
                    ...
                    if st.button('確定', key="1"):
                        if option[0] == "A":
                            ...
                            print("選了B")
                            st.session_state["current_guidance_steps"] =  st.session_state["current_guidance_steps_C"]
                            st.session_state["current_passing_conditions"] =  st.session_state["current_passing_conditions_C"]
                            st.session_state["current_guidance_choice"] = "C"
                            self.choice_made = True
                elif st.session_state["current_solution_stage"] == 1:
                    # Check whether the calculation is for the first two fractions first or for all fractions
                    st.markdown(
                        """
                        <style>
                        /* Reduce bottom margin of the header text */
                    ...
                    option = st.radio(
                        ' ',
                        ('', 'A. 先計算首兩個分數', 'B. 同時計算三個分數'),
                        index=0
                    )
                    if st.button('確定', key="2"):
                        if option[0] == "A":
                            print("選了AA")
                            st.session_state["current_guidance_steps"] = st.session_state["current_guidance_steps_A"]
                            st.session_state["current_passing_conditions"] = st.session_state["current_passing_conditions_A"]
                            st.session_state["current_guidance_choice"] = "A"
                            self.choice_made = True
                        elif option[0] == "B":
                            print("選了AB")
                            st.session_state["current_guidance_steps"] = st.session_state["current_guidance_steps_B"]
                            st.session_state["current_passing_conditions"] = st.session_state["current_passing_conditions_B"]
                            st.session_state["current_guidance_choice"] = "B"
                            self.choice_made = True
            elif st.session_state["current_num_guidance"] == 2:
                # Check whether the calculation is for the first two fractions first or for all fractions
                st.markdown(
                    """
                    <style>
                ...
                if st.button('確定', key="3"):
                    if option[0] == "A":
                        ...
                        st.session_state["current_guidance_choice"] = "B"
                        self.choice_made = True
            elif st.session_state["current_num_guidance"] == 1:
                st.session_state["current_guidance_steps"] = st.session_state["current_guidance_steps_A"]
                st.session_state["current_passing_conditions"] = st.session_state["current_passing_conditions_A"]
                st.session_state["current_guidance_choice"] = "A"
                self.choice_made = True
            if self.choice_made:
                # Clear sidebar content when choice is made (keep sidebar open but blank)
                st.markdown(
                    """
                    <style>
                    /* Clear sidebar content when choice is made - keep sidebar open but blank */
                    ...
                    section[data-testid="stSidebar"] .stButton,
                    section[data-testid="stSidebar"] .custom-header,
                    section[data-testid="stSidebar"] div[role="radiogroup"],
                    section[data-testid="stSidebar"] .stMarkdown:has(.custom-header) {
                        display: none !important;
                    }
                    /* Keep sidebar visible but clear its content */
                ...
                current_guidance_step_index = st.session_state["current_guidance_step_index"]  # get the current guidance step index
                guidance_step = st.session_state["current_guidance_steps"][current_guidance_step_index]  # get the current guidance step
                agent_text = guidance_step
                st.session_state["history"].append({"role": "assistant", "content": agent_text})
                # translate the agent's response from simplified Chinese into traditional Chinese
                agent_text_display = self.sync_translate_tw(agent_text)
                st.session_state["messages_display"].append({"role": "assistant", "content": agent_text_display})
                st.session_state["messages"].append({"role": "assistant", "content": agent_text})
                # store the agent's text for checking student understanding of the guidance step
                st.session_state["check_messages_for_understanding"].append({"role": "assistant", "content": agent_text})
                st.session_state["is_first_user_prompt"] = False  # update the first prompt status to False
                st.rerun()
    ...
    def display_messages(self, in_sidebar=False):
        ...
        if self.choice_made:
           for i, message in enumerate(st.session_state["messages_display"]):
               # for each message in the dialogues, get the role and the message content
               role = message["role"]
               content = message["content"]

               # Set a custom avatar for each role (user or assistant)
               if role == "user":
               ...
               # Custom layout: system messages on left, user messages on right
               if role == "assistant":
           ...
           # Create four columns in sidebar: text input, submit button, spacer, voice input
           col1, col2, col3, col4 = st.columns([5, 0.5, 0.05, 0.5])
           
           # Create a container to ensure all components are on the same line
           with st.container():
               with col1:
                   # Text input column
                   if st.session_state["current_guidance_step_index"] <= len(st.session_state["current_guidance_steps"]) - 1:
                         # Check if there's voice input to populate the text input
               ...
               with col4:
                   ...
                   if st.session_state["current_guidance_step_index"] <= len(st.session_state["current_guidance_steps"]) - 1:
                         ...
                         # The voice input function now handles text input updates internally
                         # When speech is recognized, text appears in the input field


    # Process user input in the chat window (input_key is either text_input or speech_input)
    def process_input(self, input_key):
        ...
        if user_input:
            ...
            guidance_step = st.session_state["current_guidance_steps"][current_guidance_step_index]
            # Retrieve the specific condition for passing a guidance step
            passing_condition = st.session_state["current_passing_conditions"][current_guidance_step_index]
            st.session_state["check_messages_for_understanding"].append({"role": "user", "content": user_text})
            check_messages = st.session_state["check_messages_for_understanding"]

            # Call the agent to check whether to move forward to the next guidance step based on student understanding
            print("循環中：當前的輔導步驟：" + str(current_guidance_step_index))
            with st.session_state["thinking_spinner"], st.spinner("處理中，請稍候 ..."):
               while agent.check_move_to_next_guidance_step(question, guidance_step, passing_condition, check_messages):
                   # if reaching the end of all guidance steps
                   if st.session_state["current_guidance_step_index"] >= len(st.session_state["current_guidance_steps"])-1:
                       end_message = "真棒！你已經完成了這道題的輔導！"
                       st.session_state["messages_display"].append({"role": "user", "content": user_text})
                       st.session_state["messages_display"].append({"role": "assistant", "content": end_message})
                       st.rerun()
                       return
                   else:
                       st.session_state["current_guidance_step_index"] += 1  # move to the next guidance step
                       current_guidance_step_index = st.session_state["current_guidance_step_index"]  # get the current guidance step index
                       print("循環中：當前的輔導步驟：" + str(current_guidance_step_index))
                       guidance_step = st.session_state["current_guidance_steps"][current_guidance_step_index]
                       passing_condition = st.session_state["current_passing_conditions"][current_guidance_step_index]

               print("出循環：當前的輔導步驟：" + str(current_guidance_step_index))
               # if the student has answered the tutor's question
               if agent.check_guidance_question_answered(check_messages, guidance_step):
                   print("有回答开始纠错")
                   # call the agent tutor to further check whether student answer is incorrect
                   st.session_state["history"], agent_text = agent.tutor(user_text, question, guidance_step, st.session_state["history"])
                   agent_text_display = self.sync_translate_tw(agent_text)
                   st.session_state["messages_display"].append({"role": "user", "content": user_text})
                   st.session_state["messages_display"].append({"role": "assistant", "content": agent_text_display})
                   ...
                   st.session_state["check_messages_for_understanding"] = []   # Reset the messages for checking understanding
                   agent_text = guidance_step
