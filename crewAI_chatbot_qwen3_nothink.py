...
#     api_key = "sk-0fefbcaf35c44ecb9d8a8ed8dfa0048f",
...
def get_llm_response(text="", history=[], header="", model=llm, system=None):
    ...
    client = OpenAI(
    # 若没有配置环境变量，请用百炼API Key将下行替换为：api_key="sk-xxx",
    api_key = "sk-f050bfda9f4641bea4043970c5cd82ab",
    base_url="https://dashscope-intl.aliyuncs.com/compatible-mode/v1",
    )
    completion = client.chat.completions.create(
        model = model,
...
def get_llm_feedback(text="",model="qwen3-max"):
...
2. 如果学生提出疑问则视为【有回答】
3. 没有明确回答**当前引导步骤**时视为【无回答】
4. 请以尽量简短的话语来进行逐步思考，不要太过冗长并在**最后生成【无回答】或【有回答】**
...
def agent_check_move_to_next_explanation(question, explanation, scale, check_messages, model="qwen3-max"):
    conversation = ""
    messages = []
    start_time = time.time()
    for h in check_messages:
        if h['role'] == 'user':
            ...
            conversation += f"教师: {h['content']}\n"
    content = f'''以下是题目，当前引导以及教师与学生的对话，请逐步思考，然后根据评判标准判断学生是否掌握当前引导,**在结尾生成【已掌握】或【未掌握】**：
## 注意：
...
6. 当学生提出疑问时，则说明未掌握，当学生对辅导表示认同时，则视为已掌握
...
3. 你是一位小学数学教师，而不是AI助手，请以**简练的语言辅导小学生，对话必须简短明了**，只需要以教师的口吻输出辅导，不要输出其他
...
def agent_check_is_incorrect_answer(history, question, scale):
    teacher_dialog, student_dialog = retrieve_dialog(history)
    start_time = time.time()
    # numbers = []
...
def agent_tutor(user_input, question, explanation, history, scale):
    system = SYSTEM
    history.append({"role": "user", "content": user_input})
    header = ""
    # Check if a valid conversation history (i.e. with a minimum length of 2)
    if len(history) >= 2:
        is_incorrect_answer = agent_check_is_incorrect_answer(history, question, scale)  # Check if the answer is incorrect
